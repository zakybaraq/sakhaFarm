 ...
 */
export async function listUsers(filters?: ListUsersFilters): Promise<UserWithRelations[]> {
  ...
  if (conditions.length === 1) {
    const result = await query.where(conditions[0]).orderBy(desc(users.createdAt))
    return result
  }
 ...
 */
export async function searchUsers(query: string, filters?: ListUsersFilters): Promise<UserWithRelations[]> {
  ...
  if (conditions.length === 1) {
    const result = await baseQuery.where(conditions[0]).orderBy(desc(users.name))
    return result
  }
 ...
 */
export async function importUsersFromCSV(csvContent: string, defaultTenantId: number) {
  const lines = csvContent.trim().split('\n')
  if (lines.length < 2) {
    throw new CsvImportError(1, 'CSV must contain a header row and at least one data row')
  }
  ...
  if (emailIdx === -1 || nameIdx === -1 || roleIdIdx === -1) {
    throw new CsvImportError(1, 'CSV must contain email, name, and role_id columns')
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      const tempPassword = generateTempPassword()
      await createUser({ email, password: tempPassword, name, roleId, tenantId: defaultTenantId })
      created.push(email)
    } catch (err) {
