...
export const feedController = new Elysia({ prefix: '/api/feed' })
  ...
  .post(
    ...
    async (ctx) => {
      ...
      const product = await createFeedProduct(ctx.body, currentTenantId, userId)
  ...
  .get(
    ...
    async (ctx) => {
      const currentTenantId = getTenantId(ctx)
      const product = await getFeedProduct(parseInt(ctx.params.id, 10), currentTenantId)
      return { product }
    },
    {
      beforeHandle: requirePermission('feed.read'),
  ...
  .put(
    ...
    async (ctx) => {
      ...
      await updateFeedProduct(parseInt(ctx.params.id, 10), ctx.body, currentTenantId, userId)
      return { success: true }
    },
    {
  ...
  .delete(
    ...
    async (ctx) => {
      ...
      await softDeleteFeedProduct(parseInt(ctx.params.id, 10), currentTenantId, userId)
      return { success: true }
    },
    {
  ...
  .get(
    ...
    async (ctx) => {
      ...
      if (ctx.query.plasmaId && ctx.query.feedProductId) {
        const stock = await getStockForPlasmaFeed(ctx.query.plasmaId, ctx.query.feedProductId, currentTenantId)
        return { stock }
      }

      const stocks = await getAllStock(currentTenantId, ctx.query.plasmaId, ctx.query.feedProductId)
      return { stocks }
    },
    {
      beforeHandle: requirePermission('feed.read'),
  ...
  .post(
    ...
    async (ctx) => {
      ...
      const result = await recordFeedConsumption(ctx.body, currentTenantId, userId)
      return result
    },
    {
      beforeHandle: requirePermission('feed.create'),
