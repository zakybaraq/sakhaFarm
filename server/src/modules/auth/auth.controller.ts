...
const loginRateLimit = new Elysia({ name: "login-rate-limit" }).onBeforeHandle(
  async ({ request, set }) => {
    const ip =
...
export const authController = new Elysia({ prefix: "/api/auth" })
  ...
  .post(
    "/register",
    async ({ body, cookie }) => {
      const { userId, sessionCookie } = await registerUser(
        body.email,
        body.password,
        body.name,
        body.roleId,
  ...
  .post(
    "/login",
    async ({ body, cookie }) => {
      const { user, sessionCookie } = await loginUser(
        body.email,
        body.password,
      );
      cookie[sessionCookie.name].set({
  ...
  .get("/me", async ({ cookie, set }) => {
    ...
    const attrs = user as unknown as LuciaUserAttributes;
    return {
      user: {
        id: user.id,
        email: attrs.email,
  ...
  .get("/permissions", async ({ cookie, set }) => {
    ...
    const attrs = user as unknown as LuciaUserAttributes;
    const permissions = await getRolePermissions(attrs.roleId);
    return { permissions };
  })
  ...
  .delete("/sessions/:id", async ({ cookie, params, set }) => {
    ...
    const userAttrs = user as unknown as LuciaUserAttributes;
    if (
      userSessions[0].userId !== user.id &&
      userAttrs.roleId !== SUPER_ADMIN_ROLE_ID
    ) {
      set.status = 403;
