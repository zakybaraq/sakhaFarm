...
export const feedBrandsController = new Elysia({ prefix: "/api/feed-brands" })
  ...
  .post(
    ...
    async (ctx: any) => {
      ...
      const feedBrand = await createFeedBrand(
        ctx.body,
        currentTenantId,
        userId,
      );
  ...
  .get(
    ...
    async (ctx: any) => {
      const currentTenantId = getTenantId(ctx);
      const feedBrand = await getFeedBrand(
  ...
  .put(
    ...
    async (ctx: any) => {
      ...
      const result = await toggleFeedBrand(
        parseInt(ctx.params.id, 10),
        currentTenantId,
        userId,
      );
